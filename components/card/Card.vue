<script setup></script>

<template>
  <div class="flex justify-center">
    <div class="w-[900px] h-[700px] border-2 rounded-md bg-cover bg-center brightness-100" style="background-image: url('https://i.pinimg.com/originals/e7/7a/60/e77a6068aa8bb2731e3b6d835c09c84c.gif')">
      <h1 class="text-center font-serif font-bold text-3xl">CITY NAME</h1>
      <hr />
      <div class="flex justify-between items-center text-center h-96">
        <div class="w-full">
          <p class="text-8xl p-6">16°C</p>
          <p>Partly Cloud</p>
        </div>
        <div class="w-full">
          <p>min: 12°C - max: 20°C</p>
          <p>wind: 7m/s</p>
          <p>humidity: 23%</p>
        </div>
      </div>
      <br />
      <div class="flex h-60 justify-between items-end p-2">
        <div
          class="w-40 text-center shadow-md cursor-pointer hover:-translate-y-2 border-2 rounded-md"
        >
          <p class="text-lg font-semibold">date</p>
          <div class="flex gap-2 p-1">
            <p>img</p>
            <p>min-max temprature</p>
          </div>
        </div>
        <div
          class="w-40 text-center shadow-md cursor-pointer hover:-translate-y-2 border-2 rounded-md"
        >
          <p class="text-lg font-semibold">date</p>
          <div class="flex gap-2 p-1">
            <p>img</p>
            <p>min-max temprature</p>
          </div>
        </div>
        <div
          class="w-40 text-center shadow-md cursor-pointer hover:-translate-y-2 border-2 rounded-md"
        >
          <p class="text-lg font-semibold">date</p>
          <div class="flex gap-2 p-1">
            <p>img</p>
            <p>min-max temprature</p>
          </div>
        </div>
        <div
          class="w-40 text-center shadow-md cursor-pointer hover:-translate-y-2 border-2 rounded-md"
        >
          <p class="text-lg font-semibold">date</p>
          <div class="flex gap-2 p-1">
            <p>img</p>
            <p>min-max temprature</p>
          </div>
        </div>
        <div
          class="w-40 text-center shadow-md cursor-pointer hover:-translate-y-2 border-2 rounded-md"
        >
          <p class="text-lg font-semibold">date</p>
          <div class="flex gap-2 p-1">
            <p>img</p>
            <p>min-max temprature</p>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>
;
